...
export const migrate = async ({
  ...
  const getFormsToMigrate = async (): Promise<Kobo.Form[]> => {
    ...
    const forms = await sourceSdk.v2.form.getAll({limit: 10000}).then(_ => _.results).then(f => {
  ...
  const standardizeAttachmentNames = (submission: Kobo.Submission): void => {
    submission._attachments?.forEach((attachment, i) => {
      ...
      Object.keys(submission.answers).forEach(k => {
        ...
        if (typeof submission.answers[k] === 'string' && sourceSdk.v1.submission.sanitizeFileName(submission.answers[k]) === fileName) {
  ...
  const migrateForm = async (form: Kobo.Form, index: number, total: number) => {
    ...
    spinner.update({text: `${logHead} ${makeStepper(1, 3)}  Migrating answers + attachments ${makeProgressAndPercent(completed, submissions.length)}`})

    const idx_uuid_validation = await PromisePool.withConcurrency(20).for(submissions).process(async (submission) => {
      ...
      spinner.update(`${logHead} ${makeStepper(1, 3)} Migrating answers + attachments ${makeProgressAndPercent(completed, submissions.length)}`)
      completed++
      if (res) return {uuid: res, validation: submission._validation_status.uid}
    }).then(_ => seq(_.results).compact().reduceObject<Record<Kobo.Submission.UUID, Kobo.Submission.Validation>>((item) => [item.uuid, item.validation!]))
    ...
    const idx_id_validation = await destinationSdk.v2.submission.get({formId: form.uid}).then(_ => {
      return seq(_.results).map(_ => {
        const validation = idx_uuid_validation[_._uuid]
        if (validation)
          return {_id: _._id, status: validation}
      }).filter(_ => _ !== undefined).groupBy(_ => _.status)
    })

    completed = 0
    await PromisePool.withConcurrency(1).for(Obj.entries(idx_id_validation)).process((async ([validationStatus, subs]) => {
      await updateValidation({
        formId: form.uid,
        submissionIds: subs.map(_ => _._id),
        status: validationStatus,
      })
      spinner.update(`${logHead} ${makeStepper(3, 3)} Migrating validation status ${makeProgressAndPercent(completed, submissions.length)}`)
      completed += subs.length
    }))
    spinner.success(`${logHead} ${makeStepper(4, 3)} Migrating validation status ${makeProgressAndPercent(completed, submissions.length)}`)
  }

  const forms = await getFormsToMigrate()
  await PromisePool.withConcurrency(1).for(forms).process((_, i) => migrateForm(_, i, forms.length))
  process.exit(0)
}